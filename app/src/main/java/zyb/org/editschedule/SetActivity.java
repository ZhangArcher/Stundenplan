...
//				startActivity(intent);
			...
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				...
				if(isChecked){
					if(startService(intent) != null)
						Toast.makeText(SetActivity.this, "succeed,handy ist in silent mode during lesson-time ", 3000).show();
					else{
						Toast.makeText(SetActivity.this, "unsucceed,try again", 3000).show();
						switch_quietButton.setChecked(false);
					}
				}
				else{
					if(stopService(intent))
						Toast.makeText(SetActivity.this, "succeed to cancel,handy ist in normal mode ", 3000).show();
					else{
						Toast.makeText(SetActivity.this, "unsucceed,try again", 3000).show();
	...
	protected Dialog onCreateDialog(int id, Bundle args) {
		...
		if(id == SINGLE_DIALOG){
			...
			// 设置对话框的标题
			b.setTitle("choice remind time before lesson");
...
//					System.out.println("SetActivity:" + time_choice);
					if(time_choice == 0){
						Toast.makeText(SetActivity.this, "choice remind time before lesson", 3000).show();
						...
						Toast.makeText(SetActivity.this, "succeed to setting,system remind before " + time_choice + "minuten to go to lesson", Toast.LENGTH_LONG).show();
					}
				}
			});
			//添加一个“取消”按钮
